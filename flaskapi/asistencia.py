...
def nuevo_paciente():
    try:
        data = request.json
        doc = (data.get('nombre_completo'),  data.get('nombre_completo_propietario'), data.get('especie'),
               data.get('peso'), data.get('raza'), data.get('color'), data.get('fecha_nac_mascota'), 
...
def _crear_paciente(doc):
    paciente = insertar_o_actualizar("""insert into 
        pacientes(nombre_completo, nombre_completo_propietario, especie, peso, raza, color, fecha_nac_mascota, domicilio, correo, telefono, contraseña)
...
def _actualizar_paciente(doc):
    paciente = insertar_o_actualizar("""update
        pacientes set nombre_completo = %s,
        nombre_completo_propietario = %s,
        especie = %s, peso = %s, raza = %s, color = %s,
        fecha_nac_mascota = %s, domicilio = %s, correo = %s,  
