...
def _obtener_cita(id = None):
    ...
    if id:
        cita = sql("""select 
            ...
            c.fecha_cita, 
            d.id_doctor,
            d.nombre_completo doctor,
            p.id_paciente, 
            p.nombre_completo paciente, 
            p.nombre_completo_propietario propietario,
            p.especie,
            p.raza,
            p.peso,
            p.color,
    ...
    lista_citas = sql('''select
            id_cita,d.id_doctor, p.id_paciente, d.nombre_completo doctor, p.nombre_completo paciente, p.nombre_completo_propietario propietario, fecha_cita, hora_cita 
            from citas c 
            inner join doctores d on d.id_doctor = c.id_doctor
            inner join pacientes p on p.id_paciente = c.id_paciente
            where 
